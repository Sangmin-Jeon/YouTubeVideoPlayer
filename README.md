...
* 구현한 내용
  ...
  * **Alert** 에 TextField추가하기  
    ...
    "ok" 버튼을 누르면 `TextField`에 작성한 URl에서 정규식을 이용하여 key값을 따로 분리 후 재사용성을 위하여 list배열에 저장 하였습니다.   
  ...
  * 즐겨찾기 추가   
    TableViewCell에서 `callBack메소드`를 작성하고 TableView에서 호출하여 즐겨찾기 버튼의 상태를 감지   
    `NotificationCenter`를 사용하여 해당 indexPath에 대한 영상목록값을 즐겨찾기 controller에 알려줍니다.   
    즐겨찾기 추가, 해제 >   
...
* 개선사항   
  ...
  * ~search bar를 활성화 했을때 검색결과에서 cell이 선택되지 않아 동영상재생view로 넘어가지 않는
    문제 확인~   
    `dimsBackgroundDuringPresentation` 속성을 false로 선언하여 해결
